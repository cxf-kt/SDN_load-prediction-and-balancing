...
class Mininet( object ):
    ...
    def iperf_single( self,host, udpBw='10M', period=60, port=5001):
        if not host:	
            return
        else:
            assert len( host ) == 2
        ...
        output( "%s and %s  bw=%4s  period=%4s\n" % ( client.name, server.name ,udpBw,period) )
        iperfArgs = 'iperf -u '
        bwArgs = '-b ' + udpBw + ' '
        output("***start server***")
        server.cmd( iperfArgs + '-s -i 1' + ' > /home/xf/mininet/custom/log/' + filename + '&')
        output("***start client***")
        client.cmd(
            iperfArgs + '-t '+ str(period) + ' -c ' + str(server.IP()) + ' ' + bwArgs
            +' > /home/xf/mininet/custom/log/' + 'client' + filename +'&')

    def iperfMulti(self, bw_mu=3,bw_sigma=0.01, period_mu=1,period_sigma=1 ,time_out=120,sleep_time=10):
        host_list = [h for h in self.hosts]
        base_port=5001

        timeout = time.time() + float(time_out)*60   

        while True:
            if time.time() > timeout:
                break
                output("the time has out of setting")
            _len=len(host_list)
            for i in range(0,_len):
                ...
                while server == client:
                    server=random.choice(host_list)
                bw = random.gauss(float(bw_mu),float(bw_sigma))
                period = random.gauss(float(period_mu),float(period_sigma))
                while bw<=0 or period<=0:
                    bw = random.gauss(float(bw_mu),float(bw_sigma))
                    period = random.gauss(float(period_mu),float(period_sigma))
                bw=str(bw)+'M'
                self.iperf_single(port=base_port,period=period, udpBw=bw,host=[client, server])
                sleep(period/2)
            sleep(float(sleep_time))
        sleep(period)
        output("iperfmulti has done")
    ...
    def iperf_single_pd( self,hosts=None, udpBw='10M', period=60, port=5001):
        ...
        bwArgs = '-b ' + udpBw + ' '
        output("***start server***")
        server.cmd( iperfArgs + '-s -i 1' + ' ~/mininet/custom/log/' + filename + '&')
        output("***start client***")
        client.cmd(iperfArgs + '-t '+ str(period) + ' -c ' + server.IP() + ' ' + bwArgs
            +' > ~/mininet/custom/log/' + 'client' + filename +'&')
    def iperfPb (self, bw, period = 60, i = 1,j = 4,k = 64,pt = 0.5,pa = 0.3):
        ...
        for key in range(_len):
            ...
            self.iperf_single_pd(hosts = [client, server], udpBw=bw, port=base_port)
            sleep(.05)
        sleep(period)
