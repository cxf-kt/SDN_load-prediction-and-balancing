...
class CLI( Cmd ):
    ...
    def do_iperfmulti( self, line ):
            ...
            if len(args) ==2:
                time_out= args[ 0 ]
                bw_mu=args[1]
                self.mn.iperfMulti(time_out=time_out,bw_mu=bw_mu)
            elif len(args)>2:
                bw_mu= args[ 0 ]
                bw_sigma= args[ 1 ]
                period_mu= float(args[ 2 ])
                period_sigma= float(args[3])
                time_out = int(args[ 4 ])
                sleeptime = int(args[5])
                self.mn.iperfMulti(bw_mu=bw_mu,bw_sigma=bw_sigma,period_mu=period_mu,period_sigma=period_sigma,time_out=time_out,sleep_time=sleeptime)
