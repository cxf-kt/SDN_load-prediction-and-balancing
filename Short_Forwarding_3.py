...
class NetworkAwareness(app_manager.RyuApp):
    ...
    def _monitor(self):
        ...
        while True:
            ...
            if self.weight_length==5:
                ...
                df.to_csv("~/ryu/ryu/app/GCN/var3_8.csv",mode='a',header=False,index=False)
                self.flows=[]
                self.weight_sum=[]
                #weight_input = []
                self.weight_length=0
            ...
            self.flows.append(weights)
            #self.weight_sum.append(np.sum(weight_input))
            #print('free_bw:',free_bw)
            #print('weight_input',weight_input)
            #print('flow_input:',self.flow_input)
            #print('weight_sum',self.weight_sum)
            self.weight_length+=1
            print('self.weight_length:',self.weight_length)

            self.flow_input.append(free_bw.reshape((lens*lens)))
            #print(free_bw.reshape((lens*lens)))
            if len(self.flow_input)==10:
                weight_input=self.link_bw_dict
                X,means,stds=self._data_process(self.flow_input)
                out=self._forecast(X)
                # unnormolize
                out_unnormalized = out.detach().cpu().numpy() * stds+ means
                data=out_unnormalized[0,9,:]
                k=0
                #print('weight',weight_input)
                for i in range(lens):
                    for j in range(lens):
                        if weight_input[i][j]>=0:
                            if weight_input[i][j]>data[k]:
                                if data[k]>=0:
                                    weight_input[i][j]=data[k]     
                            k+=1
                flow_max=np.max(weight_input)
                self._set_graph(weight_input,flow_max,lens)
                self.flow_input=[]
            ...
            #print('Link variance:',freebw_var)
            #df = pd.DataFrame(np.expand_dims(list(freebw_var.reshape((1*1))), axis=0))
            #df.to_csv("~/ryu/ryu/app/GCN/var3_2.csv",mode='a',header=False,index=False)
            hub.sleep(1)

    def _set_graph(self,free_bw,freebw_max,lens):
    ...
    def _data_process(self,data):
        ...
        data=data-mean
        data=data/std
        input=torch.from_numpy(data.reshape((1,10,20))).float().to(device=self.device)
        return input,mean,std

    def _forecast(self,X):
        lstm = torch.load('/home/sa/ryu/ryu/app/LSTM/lstm5.pkl',map_location=self.device)
        net=LSTM_linear_(10,10,20)
        net.load_state_dict(lstm)
        net.eval
        out=net(X)
        return out
